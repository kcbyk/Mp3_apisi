...
def _loader_baslat(url):
    """loader.to donusumu baslatir; (progress_url, hata) dondurur."""
    ...
    try:
        r = requests.get("https://loader.to/ajax/download.php",
                         params={"format": "mp3", "url": url}, headers=h, timeout=20).json()
    except Exception as ex:
...
def _ruvs_baslat(url):
    ...
    try:
        r = requests.post("https://www.ruvs.in/api/convert",
                          data=json.dumps({"url": url, "format": "mp3", "quality": "320"}),
...
def _yt_dl_url_bul(url, ilerleme=None):
    ...
    purl, hata = _loader_baslat(url)
    if not purl:
        return None
    return _loader_bekle(purl, ilerleme)

def yt_indir(url, baslik, ilerleme=None, sure=0):
    """YouTube -> mp3 (320kbps). Once ruvs.in (hizli), yedek loader.to. Pre-warm destegi."""
    dl = None
    pw = PREWARM.get(url)
    if pw and time.time() - pw["t"] < PREWARM_TTL:
        if pw.get("dl"):
            dl = pw["dl"]
            if ilerleme:
    ...
    if not dl:
        dl = _yt_dl_url_bul(url, ilerleme)
    if not dl:
